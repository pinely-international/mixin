...
describe('Using mixins that share a common ancestor', () => {
	forEachSettings(() => {
		class Base {
			methodA() { return 'a' }
			methodB() { return 'b' }
		 ...
		 * 		Mixed#methodC() -> 'c'
		 */
		it('should not apply the shared ancestor more than once', () => {
			expect(m.methodA()).not.toBe('a');	// re-override test
			expect(m.methodA()).toBe('A');			// what it should be

			expect(m.methodB()).toBe('B');
			expect(m.methodC()).toBe('c');
		});
	});
});
